...
export default function ModulePage({ mod, notes, onBack, onFiche, onNewFiche, onDeleteNote }) {
  ...
  const handleDelete = async () => {
    ...
    setConfirmDel(null)
    setTimeout(async () => {
      await onDeleteNote(confirmDel.id)
      setDeleting(null)
    }, 300)
  }
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 mb-5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <motion.button
            whileTap={{ scale: .95 }}
            onClick={onNewFiche}
            className="px-4 py-2 text-white text-sm font-medium rounded-xl transition-all hover:brightness-90"
            ...
            {filtered.map(n => {
              ...
              return (
                <motion.div
                  ...
                  whileHover={{ y: -2, boxShadow: '0 6px 20px rgba(0,0,0,.08)' }}
                  whileTap={{ scale: .98 }}
                >
                  {/* Accent bar */}
                  ...
                  <div className="flex items-start justify-between gap-2 mt-1 mb-1.5">
                    <p className="text-sm font-semibold text-gray-800 dark:text-zinc-200 leading-snug flex-1">{n.title}</p>
